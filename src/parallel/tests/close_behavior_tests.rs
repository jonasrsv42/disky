...
use crate::parallel::writer::{ParallelWriter, ParallelWriterConfig, ShardingConfig as WriterShardingConfig};
...
fn test_reader_close_fulfills_pending_promises() -> Result<()> {
    ...
    for _ in 0..10 { // Only 5 records exist
...
fn test_reader_close_fulfills_pending_drain_promises() -> Result<()> {
    ...
    let mut promises = Vec::new();
    for _ in 0..3 { // More drains than shards
...
fn test_writer_close_fulfills_pending_write_promises() -> Result<()> {
    ...
    let sharding_config = WriterShardingConfig::new(
        Box::new(sharder),
        1
    );
    
    // Create writer with default config
    let writer = ParallelWriter::new(
        sharding_config,
    ...
    let task_thread = std::thread::spawn(move || -> Result<()> {
        ...
        for promise in promises {
            match promise.wait() {
                ...
                other => {
                    return Err(DiskyError::Other(format!("Unexpected promise result: {:?}", other)));
                }
            }
        }
