...
impl SimpleChunkParser {
    ...
    /// * `chunk_data` - the buffer containing chunk data (after the 40-byte header)
    ///
    /// # Returns
    ///
    ...
    /// # Returns
    ///
    /// - SimpleChunkPiece::Record(Bytes) if a record was read
    /// - SimpleChunkPiece::EndOfChunk if there are no more records, the input buffer has been advanced
    /// - Err if an error occurred or if next() is called after EndOfChunk was returned
    pub fn next(&mut self) -> Result<SimpleChunkPiece> {
        match self.state {
            ReaderState::Ready => {
            ...
            ReaderState::Reading => {
                ...
                if self.records_read >= self.header.num_records {
                    ...
                    return Ok(SimpleChunkPiece::EndOfChunk);
                }
                ...
                Ok(SimpleChunkPiece::Record(record))
            }
            ReaderState::EndOfChunk => Err(DiskyError::Other(
