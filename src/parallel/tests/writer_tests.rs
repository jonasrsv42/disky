...
fn test_parallel_writer_basic() {
    ...
    let parallel_writer =
        Arc::new(ParallelWriter::new(sharding_config, ParallelWriterConfig::default()).unwrap());
...
fn test_sync_operations() {
    ...
    // Create parallel writer
    let parallel_writer =
        ParallelWriter::new(sharding_config, ParallelWriterConfig::default()).unwrap();
...
fn test_bytes_written_tracking() {
    ...
    resource_pool
        .process_all_resources(|resource| {
            ...
            assert_eq!(
                resource.bytes_written, expected_bytes,
...
fn test_writer_rotation_on_byte_limit() {
    ...
    for i in 0..9 {
        ...
        println!(
            "Available writers after record {}: {}",
            i,
            parallel_writer.available_resource_count().unwrap()
        );
    ...
    assert!(
        parallel_writer.available_resource_count().unwrap() < 3,
...
fn test_partial_writer_rotation_on_byte_limit() {
    ...
    assert!(
        parallel_writer.write_record(record4).is_ok(),
