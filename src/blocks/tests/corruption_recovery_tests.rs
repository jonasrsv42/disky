...
fn test_recover_from_corrupted_header() {
    ...
    assert!(matches!(result.unwrap_err(), DiskyError::BlockHeaderHashMismatch));
    ...
    assert!(!recovered_chunk.is_empty(), "Should recover to a non-empty chunk");
    assert_eq!(&recovered_chunk[..], chunk3, "Should recover to the third chunk");
}
...
fn test_recover_from_multiple_corrupted_headers() {
    ...
    assert!(file_data.len() > (2 * block_size) as usize, 
            "File not large enough for test, expected > {} bytes, got {} bytes",
            2 * block_size, file_data.len());
    ...
    assert!(file_data.len() > (first_boundary_pos + 8) as usize,
    ...
    file_data[first_boundary_pos as usize + 1] = file_data[first_boundary_pos as usize + 1].wrapping_add(1);
    ...
    // Make sure it's a valid position in the file
    assert!(file_data.len() > (second_boundary_pos + 8) as usize,
            "File too small to corrupt second header, len: {}, need: {}",
            file_data.len(), second_boundary_pos + 8);
            
    // Corrupt the second header
    file_data[second_boundary_pos as usize] = file_data[second_boundary_pos as usize].wrapping_add(1);
    file_data[second_boundary_pos as usize + 1] = file_data[second_boundary_pos as usize + 1].wrapping_add(1);
...
fn test_recover_at_end_of_file() {
    ...
    let buffer = Cursor::new(Vec::new());
    let mut writer = BlockWriter::with_config(buffer, BlockWriterConfig { block_size }).unwrap();
    writer.write_chunk(Bytes::from(b"Single chunk".to_vec())).unwrap();
    writer.flush().unwrap();
    let mut file_data = writer.into_inner().into_inner();
    ...
    assert!(matches!(recovery_result.unwrap_err(), DiskyError::UnexpectedEof));
}
