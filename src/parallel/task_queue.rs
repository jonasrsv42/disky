...
impl<T> TaskQueue<T> {
    ...
    pub fn push_back(&self, t: T) -> Result<()> {
        ...
        if inner.state == TaskQueueState::Closed {
            return Err(DiskyError::QueueClosed("Cannot add to a closed queue".to_string()));
        }
    ...
    pub fn read_front(&self) -> Result<T> {
        ...
        loop {
            ...
            if inner.state == TaskQueueState::Closed {
                return Err(DiskyError::QueueClosed("Queue is closed and empty".to_string()));
            }
    ...
    pub fn get_state(&self) -> Result<TaskQueueState> {
    ...
    pub fn is_closed(&self) -> Result<bool> {
        ...
        Ok(inner.state == TaskQueueState::Closed)
    }
...
mod tests {
    ...
    fn test_waiting_for_tasks() {
        ...
        // Sleep a bit to ensure the thread is waiting
