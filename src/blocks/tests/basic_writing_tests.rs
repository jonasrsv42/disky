...
fn test_write_chunk_smaller_than_block() {
    ...
    assert!(
        chunk_data.len() as u64 + BLOCK_HEADER_SIZE < block_size,
...
fn test_position_tracking_when_appending() {
    ...
    let mut writer =
        BlockWriter::for_append_with_config(buffer, initial_position, BlockWriterConfig::default())
            .unwrap();
