...
fn test_repeated_signatures() {
    ...
    let mut combined =
        BytesMut::with_capacity(buffer.len() + signature_bytes.len() + second_part.len());
    ...
    for (idx, record) in records.iter().enumerate().take(10) {
        // At most 10 records
        // Record data should be consistent
        assert!(
            record.iter().all(|&b| b == record[0]),
            "Record data inconsistent at index {}",
            idx
        );
        ...
        if idx < 5 {
            ...
            assert_eq!(
                record[0], idx as u8,
                "First set record {} has wrong value",
                idx
            );
        } else if idx < 10 {
    ...
    println!(
        "Read {} records with {} errors from file with repeated signatures",
        records.len(),
        errors.len()
    );
}
...
fn test_recovery_at_file_boundaries() {
    ...
    assert!(
        buffer.len() > 256,
        "Test file must be at least 2 blocks for this test"
    );
    ...
    println!(
        "Read {} records from boundary-corrupted file",
        records.len()
    );
    ...
    assert!(
        !records.is_empty(),
        "Expected some records despite corruption"
    );
