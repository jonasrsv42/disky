...
fn test_parallel_writer_round_trip() -> Result<()> {
    ...
    let parallel_writer = ParallelWriter::new(sharding_config, ParallelWriterConfig::default())?;
    ...
    parallel_writer
        ...
        .process_all_resources(|resource| {
            ...
            let cursor: &Cursor<Vec<u8>> = resource.writer.get_block_writer().get_ref();
            // Clone the data
    ...
    for expected_record in &test_records {
        match reader.next_record()? {
            DiskyPiece::Record(bytes) => {
                assert_eq!(
                    bytes,
                    Bytes::from(expected_record.clone()),
