...
fn test_failure_during_header_write() {
    ...
    match result {
        Err(crate::error::RiegeliError::Io(_)) => {
            ...
            let sink = writer.get_ref();
            assert_eq!(sink.written, 10, "Should have written 10 bytes before failing");
...
fn test_flush_error_handling() {
    ...
    match result {
        Err(crate::error::RiegeliError::Io(_)) => {
            ...
            let sink = writer.get_ref();
            assert!(sink.data.len() > 0, "Data should have been written despite flush error");
        }
        _ => panic!("Expected I/O error but got {:?}", result),
    }
}
...
fn test_incremental_writing() {
    ...
    while bytes_written < CHUNK_SIZE {
        ...
        let increment = Bytes::from(complete_data[bytes_written..bytes_written + next_size].to_vec());
