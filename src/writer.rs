...
//    COMPRESSION_TYPE_NONE, FILE_SIGNATURE,
//};
...
//    // Size of Riegeli block in bytes
//    pub block_size: u64,
//    // Minimal size of chunk before we flush it.
...
//pub struct RecordWriter<SinkType: Write + Seek, ChunkWriterType: ChunkWriter = SimpleChunkWriter> {
...
//}
//
//impl<SinkType: Write + Seek> RecordWriter<SinkType, SimpleChunkWriter> {
//    /// Creates a new RecordWriter with default options.
//    pub fn new(writer: SinkType) -> Result<Self> {
//        Self::with_options(writer, RecordWriterOptions::default())
//    }
//
//    /// Creates a new RecordWriter with the specified options.
//    pub fn with_options(writer: SinkType, options: RecordWriterOptions) -> Result<Self> {
...
//}
//
//impl<SinkType: Write + Seek, ChunkWriterType: ChunkWriter> RecordWriter<SinkType, ChunkWriterType> {
//    /// Writes a record to the file.
//    pub fn write_record(&mut self, record: &[u8]) -> Result<RecordPosition> {
//        if self.closed {
//            return Err(RiegeliError::WritingClosedFile);
//        }
//        let RecordsSize(size) = self.chunk_writer.write_record(record)?;
//
//        // If the chunk is full, write it
//        if size >= self.options.min_chunk_size {
//            self.flush_chunk()?;
//        }
//
...
//        self.writer
//            .seek(SeekFrom::Start(next_block_pos - BLOCK_HEADER_SIZE as u64))?;
//
//        // Write the header
//        self.writer.write_all(&final_header)?;
//
