...
use crate::parallel::sharding::{
    Autosharder, FileSharder, FileSharderConfig, RandomRepeatingFileShardLocator, ShardLocator,
    Sharder,
};

// Helper function to create a simple memory-based sharder
fn create_memory_sharder() -> impl Sharder<Cursor<Vec<u8>>> {
    Autosharder::new(|| Ok(Cursor::new(Vec::new())))
}
    ...
    let sharder = Autosharder::new(move || -> Result<Cursor<Vec<u8>>> {
        ...
        *count += 1;
        ...
        if *count > 2 {
            ...
            Err(crate::error::DiskyError::Other(
...
fn test_file_sharder_with_append_mode() {
    ...
    for i in 0..3 {
        ...
        file.write_all(format!("existing file {}", i).as_bytes())
            .unwrap();
    }
...
fn test_random_repeating_file_shard_locator_with_empty_directory() {
    ...
    let result = RandomRepeatingFileShardLocator::new(dir_path.clone(), "empty");
...
fn test_random_repeating_file_shard_locator_two_threads() {
    ...
    for i in 0..10 {
        ...
        let mut file = std::fs::File::create(&file_path).unwrap();
        file.write_all(format!("thread test {}", i).as_bytes())
            .unwrap();
    }
    ...
    let all_contents: std::collections::HashSet<_> =
        contents1.iter().chain(contents2.iter()).collect();
    assert_eq!(
        all_contents.len(),
        10,
        "There should be exactly 10 unique file contents"
    );
}
