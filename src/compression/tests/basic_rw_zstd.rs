...
fn test_zstd_roundtrip_small_data() {
    ...
    let decompressed = decompressor
        .decompress(compressed_bytes, original_data.len())
...
fn test_zstd_roundtrip_large_data() {
    ...
    assert!(
        compressed.len() < original_bytes.len() / 10,
    ...
    let compressed_bytes = Bytes::copy_from_slice(compressed);
    let decompressed = decompressor
        .decompress(compressed_bytes, original_bytes.len())
        .expect("Decompression failed");
...
fn test_zstd_multiple_compressions() {
...

    {
        ...
        let compressed_bytes = Bytes::copy_from_slice(compressed);
        let decompressed = decompressor
            .decompress(compressed_bytes, data1.len())
            .expect("Decompression failed");
...

    {
        ...
        let compressed_bytes = Bytes::copy_from_slice(compressed);
        let decompressed = decompressor
            .decompress(compressed_bytes, data2.len())
            .expect("Decompression failed");
...

    {
        ...
        let compressed_bytes = Bytes::copy_from_slice(compressed);
        let decompressed = decompressor
            .decompress(compressed_bytes, data3.len())
            .expect("Decompression failed");
...
fn test_zstd_different_compression_levels() {
    ...
    for level in [1, 3, 6, 9, 15] {
        let mut compressor = ZstdCompressor::with_level(level).expect(&format!(
            "Failed to create ZstdCompressor with level {}",
            level
        ));
        let mut decompressor = ZstdDecompressor::new();
        ...
        assert_eq!(
            decompressed.as_ref(),
            original_bytes,
            "Roundtrip failed with compression level {}",
            level
        );
    }
}
...
fn test_zstd_invalid_compression_level() {
    ...
    assert!(
        ZstdCompressor::with_level(0).is_err(),
        "Level 0 should be invalid"
    );
    assert!(
        ZstdCompressor::with_level(23).is_err(),
        "Level 23 should be invalid"
    );
    assert!(
        ZstdCompressor::with_level(-1).is_err(),
        "Negative level should be invalid"
    );
}
...
fn test_zstd_e2e_writer_reader_roundtrip() {
    ...
    println!(
        "Compressed size: {} bytes, Uncompressed data: {} bytes",
        buffer.len(),
        uncompressed_size
    );
    assert!(
...
fn test_zstd_e2e_mixed_record_sizes() {
    ...
    let test_records = vec![
        ...
        "medium sized record with some text content"
            .repeat(10)
            .as_bytes()
            .to_vec(),
        "large record with lots of repeated content "
            .repeat(100)
            .as_bytes()
            .to_vec(),
    ...
    // Read back and verify
    {
        ...
        for (i, (original, read_back)) in test_records.iter().zip(read_records.iter()).enumerate() {
            assert_eq!(
                original,
                read_back,
                "Record {} content mismatch (size: original={}, read={})",
                i,
                original.len(),
                read_back.len()
            );
