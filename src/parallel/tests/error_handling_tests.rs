...
fn test_async_reading_across_shards() -> Result<()> {
    ...
    assert_eq!(
        shard_finished_count + eof_count,
        1,
...
fn test_byte_queue_drain_multiple_resources() -> Result<()> {
    ...
    assert_eq!(
        second_record_count, 2,
        "Expected 2 records from second shard"
    );
...
fn test_async_drain_multiple_resources() -> Result<()> {
    ...
    assert!(
        promise.wait().is_ok(),
        "Expected successful third drain with EOF"
    );
...
fn test_reading_after_corruption() -> Result<()> {
    ...
    assert!(
        result.is_err(),
        "Expected error from corrupted second shard"
    );
    ...
    assert!(
        result.is_ok(),
        "Expected success on third shard after corruption"
    );
    ...
    assert_eq!(
        records.len(),
        2,
        "Expected 2 records from third shard after corruption"
    );
...
fn test_async_reading_with_corruption() -> Result<()> {
    ...
    // First drain should succeed
    assert!(
        first_promise.wait().is_ok(),
        "Expected successful first drain"
    );

    // Second drain should fail
    let second_result = second_promise.wait();
    assert!(
        second_result.is_ok() && second_result.unwrap().is_err(),
        "Expected error in second drain"
    );

    // Third drain should succeed even after second one failed
    assert!(
        third_promise.wait().is_ok(),
        "Expected successful third drain after corruption"
    );
