//! Writer for Disky simple chunk format.
//!
//! This module provides functionality for creating and writing simple chunks
//! according to the Disky file format specification.
...
use crate::compression::{Compressor, create_compressor};
use crate::error::Result;
use crate::hash::highway_hash;
use crate::varint;
use bytes::{BufMut, BytesMut};

/// Writer for Disky simple chunks with records.
///
...
pub struct SimpleChunkWriter {
    ...
    // Compressor instance - created once and reused for efficiency
...
impl ChunkWriter for SimpleChunkWriter {
    ...
    ///
    /// According to the Disky file format, each chunk consists of:
    ...
    /// requirements of Disky files. The buffer is efficiently reused in subsequent calls.
    fn serialize_chunk(&mut self) -> Result<&[u8]> {
        ...
        self.serialized_chunk
            .extend_from_slice(&[0; CHUNK_HEADER_SIZE as usize]);
        ...
        self.serialized_chunk
            .put_u8(self.compression_type.as_byte());

        varint::write_vu64(self.sizes.len() as u64, &mut self.serialized_chunk);

        // Write the uncompressed record sizes
        self.serialized_chunk.extend_from_slice(&self.sizes);

        // Compress and write the record values
        let compressed_records = self.compressor.compress(&self.records)?;
        self.serialized_chunk.extend_from_slice(compressed_records);
...
impl SimpleChunkWriter {
    ...
    pub fn new(compression_type: CompressionType) -> Result<Self> {
    ...
    pub fn with_chunk_size(
        ...
        // Create compressor once and reuse for efficiency
...
mod tests {
    ...
    fn test_simple_chunk_writer_construction() {
        let writer = SimpleChunkWriter::new(CompressionType::None).unwrap();
    ...
    fn test_serialize_chunk() {
        ...
        let num_records = (header_copy[25] as u64)
    ...
    fn test_multiple_chunk_serialization() {
        ...
        let num_records1 = (header1[25] as u64)
            | ((header1[26] as u64) << 8)
        ...
        let num_records2 = (header2[25] as u64)
            | ((header2[26] as u64) << 8)
        ...
        assert_eq!(
            data_hash1,
            highway_hash(&Bytes::copy_from_slice(data1_bytes))
        );
        assert_eq!(
            data_hash2,
            highway_hash(&Bytes::copy_from_slice(data2_bytes))
        );
    ...
    fn test_chunk_size_fields_correctness() {
        ...
        let mut chunk_writer = SimpleChunkWriter::new(CompressionType::None).unwrap();
        ...
        assert_eq!(
            ...
            "data_size field should be record size ({}) plus 3 bytes overhead",
            record_size
        );

        // decoded_data_size should be exactly equal to the original record size
        assert_eq!(
            decoded_data_size, record_size,
        ...
        assert_eq!(
            chunk_type,
            ChunkType::SimpleRecords as u8,
            "Chunk type should be 'r' for simple records"
        );

        let num_records = (chunk_data[25] as u64)
