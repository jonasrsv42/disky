...
fn test_write_chunk_crossing_block_boundary() {
    ...
    println!(
        "First part of chunk: bytes {} to {}",
        first_part_start, first_part_end
    );
    ...
    println!(
        "Second part of chunk: bytes {} to {}",
        second_part_start, second_part_end
    );
...
fn test_write_chunk_crossing_multiple_block_boundaries() {
    ...
    let expected_header_count = if vec.len() == expected_data_size + 3 * BLOCK_HEADER_SIZE as usize
    {
        ...
        panic!(
            "Unexpected output size: {}, expected either {} or {}",
            vec.len(),
    ...
    for i in 0..expected_header_count {
        ...
        assert_eq!(
            output_pos, header_pos,
            "Expected header at position {}, but we're at {}",
            header_pos, output_pos
        );
        ...
        let next_header_pos =
            if i < expected_header_count - 1 && (header_positions[i + 1] as usize) < vec.len() {
...
fn test_header_values() {
    ...
    let mut writer =
        BlockWriter::for_append_with_config(buffer, initial_pos, BlockWriterConfig { block_size })
            .unwrap();
    ...
    assert_eq!(
        previous_chunk, expected_previous_chunk,
    ...
    if next_chunk != expected_next_chunk {
        ...
        println!(
            "  previous_chunk: {} (expected: {})",
            previous_chunk, expected_previous_chunk
        );
        println!(
            "  next_chunk: {} (expected: {})",
            next_chunk, expected_next_chunk
        );
    }
...
fn test_compute_chunk_end() {
    ...
    let buffer = Cursor::new(Vec::new());
    let writer = BlockWriter::with_config(buffer, BlockWriterConfig { block_size: 64 }).unwrap();
    ...
    let buffer = Cursor::new(Vec::new());
    let writer = BlockWriter::with_config(buffer, BlockWriterConfig::default()).unwrap();
    ...
    let additional_headers =
        (remaining_after_first_boundary + usable_block_size - 1) / usable_block_size;
