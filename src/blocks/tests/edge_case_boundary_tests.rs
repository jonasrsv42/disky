...
fn test_chunk_end_edge_cases() {
    ...
    assert_eq!(
        end2, block_size,
        "Chunk should end at exactly the block boundary"
    );
    ...
    assert_eq!(
        end3,
        block_size + 1 + BLOCK_HEADER_SIZE,
    ...
    assert_eq!(
        end4,
        block_size + 10 + BLOCK_HEADER_SIZE,
...
fn test_chunk_starting_near_block_boundary() {
    ...
    assert_eq!(
        position, 63,
        "Position should be 1 byte before block boundary"
    );
...
fn test_chunk_ending_at_exact_block_boundary() {
    ...
    assert_eq!(
        data.len(),
        block_size as usize,
        "Data length should be exactly one block ({})",
        block_size
    );
    ...
    assert_eq!(
        next_chunk,
        usable_block_size + BLOCK_HEADER_SIZE,
        "next_chunk should be the usable block size ({})",
        usable_block_size
    );
...
fn test_multi_block_formula_validation() {
    ...
    for position in (0..data.len()).step_by(block_size as usize) {
        ...
        assert!(
            next_chunk > 0,
            "Formula 2 violation: next_chunk at position {} should be > 0",
            position
        );
